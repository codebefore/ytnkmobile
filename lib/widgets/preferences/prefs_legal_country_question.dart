...
class PrefsLegalCountryQuestion extends StatelessWidget {
  const PrefsLegalCountryQuestion({super.key});
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GlobalBloc, GlobalState>(
      ...
      builder: (context, state) => Column(
        ...
        children: [
          PrefsQuestionText(
              text: GlobalTexts.current.PREFERENCES_PAGE_legalCountryQuestion),
          const SizedBox(height: 10),
          Wrap(
            spacing: 8.0, // gap between adjacent chips
            runSpacing: 8.0, // gap
            children: [
              ...state.profile.preferences.legalCountries.map
                        ...
                        onDeleted: () {
                          context.read<GlobalBloc>().add(UpdatePreferencesEvent(
                                preferences: state.profile.preferences.copyWith(
                                  legalCountries: state
                                      .profile.preferences.legalCountries
                ...
                onPressed: () {
                  showModalBottomSheet<List<Item>>(
                    ...
                    builder: (context) => ChipSelectionSheet(
                        ...
                        maxSelectionCount: 5 -
                            state.profile.preferences.legalCountries.length),
                  ).then((value) => {
                        if (value != null)
                          {
                            if (context.mounted)
