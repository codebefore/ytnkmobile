...
class PrefsSectorQuestion extends StatelessWidget {
  const PrefsSectorQuestion({super.key});

  Future<List<Item>> _fetchChips(String filter) async {
    return FuzzySearchService().searchWithItem("sectors", filter);
  }
  ...
  Widget build(BuildContext context) {
    return BlocConsumer<GlobalBloc, GlobalState>(
      ...
      builder: (context, state) => Column(
        ...
        children: [
          PrefsQuestionText(
              text: GlobalTexts.current.PREFERENCES_PAGE_sectorsQuestion),
          const SizedBox(height: 10),
          Wrap(
            spacing: 8.0, // gap between adjacent chips
            runSpacing: 8.0, // gap
            children: [
              ...state.profile.preferences.sectors.map
                  // Display selected sectors
                  ((sector) => Chip(
                        label: Text(sector.text),
                        onDeleted: () {
                          context.read<GlobalBloc>().add(UpdatePreferencesEvent(
                                preferences: state.profile.preferences.copyWith(
                                  sectors: state.profile.preferences.sectors
                                      .where((element) => element != sector)
                                      .toList(),
                                ),
                              ));
                        },
                ...
                onPressed: () {
                  showModalBottomSheet<List<Item>>(
                    ...
                    builder: (context) => ChipSelectionSheet(
                        ...
                        maxSelectionCount:
                            5 - state.profile.preferences.sectors.length),
                  ).then((value) => {
                        if (value != null)
                          {
                            if (context.mounted)
                                ...
                                context
                                    ...
                                    .add(UpdatePreferencesEvent(
                                      preferences:
                                        ...
                                        sectors: <Item>{
                                          ...state.profile.preferences.sectors,
                                          ...value.map((e) => e),
                                        }.toList(),
                                      ),
                                    ))
