  ...
  Widget build(BuildContext context) {
    return BlocBuilder<GlobalBloc, GlobalState>(
      ...
      builder: (context, state) {
        Future<List<Item>> fetchAvailableSkills(String filter) async {
          final skills =
              await FuzzySearchService().searchWithItem("skills", filter);
          final existingSkillKeys = state.profile.skills
          ...
          return skills
              .where((skill) {
                final normalizedKey = skill.key.trim().toLowerCase();

                if (normalizedKey.isEmpty ||
                    existingSkillKeys.contains(normalizedKey) ||
                ...
                onPressed: () {
                  showModalBottomSheet<List<Item>>(
                    ...
                    builder: (context) => ChipSelectionSheet(
                        fetchChips: fetchAvailableSkills,
                    ...
                    context.read<GlobalBloc>().add(
                          AddSkillsEvent(
                            skills:
                                value.map((e) => Item(e.key, e.text)).toList(),
                          ),
                        );
                  });
                },
